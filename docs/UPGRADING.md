...
## Offline / local-first baseline (migration `0010`)

### Database

Before the first public releases, the early Alembic history (revisions
`0001`–`0009`) was collapsed into a single squashed no-op baseline, `0010`.
This only matters for development databases older than that baseline:

- **Recreate any development database from before the baseline.** The schema is built by
  ...
  carried over from such an old checkout will be missing the sync columns.
  Drop and recreate it.
- A dev database stamped at a pre-squash revision (`0001`–`0009`) must also be
  recreated — those revision ids no longer exist.
- **Since the baseline, schema changes ship as real, autogenerated Alembic
  revisions** on top of `0010` and are applied at startup.
...
The local-first delta-sync API needs nothing configured server-side, but if you
proxy or firewall the API, be aware of:
...
The frontend is **local-first (offline-capable) by default**. To run the old
online-only behaviour instead, set at build/deploy time:
