...
export default async function globalSetup(): Promise<void> {
  ...
  // Build the static frontend so the backend can serve it.
  // In UI mode, skip the build if the output already exists (saves 2-3 min on
  // subsequent runs).  Headless runs always rebuild to pick up source changes.
  // Force a rebuild in UI mode with:  FORCE_BUILD=1 ./run_e2e_tests.sh --ui
  const isUIMode = process.argv.includes("--ui");
  const needsBuild =
    !isUIMode || !existsSync(FRONTEND_OUT) || process.env.FORCE_BUILD === "1";
  if (needsBuild) {
    console.log("▶  Building frontend (nuxt generate) …");
    execSync("bunx nuxt generate", {
      cwd: FRONTEND_ROOT,
      stdio: "inherit",
    });
    console.log("✔  Frontend built");
  } else {
    console.log("✔  Frontend already built, skipping nuxt generate");
  }
